...
class TrainerById(Resource):
    def get(self, id):  # Note: parameter is 'id', not 'trainer_id'
        trainer = db.session.query(Trainer).get(id)  # Fixed: use 'id'
        if not trainer:
            return make_response({"error": "Trainer not found"}, 404)
        resp = make_response(trainer.to_dict(), 200)
        return resp

    def patch(self, id):  # Note: parameter is 'id'
        trainer = db.session.query(Trainer).get(id)  # Fixed: use 'id'
        if not trainer:
            return make_response({"error": "Trainer not found"}, 404)
        ...
        response = make_response(resp_dict, 200)  # Changed to 200 for successful update
        return response
